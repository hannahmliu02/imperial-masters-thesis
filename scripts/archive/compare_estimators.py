#!/usr/bin/env python3
"""Compare the OLD vs NEW bias estimator on an injected G_p, and plot the change.
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Old vs new bias estimator comparison.")
    ap.add_argument("--gp", required=True, help="Path to the G_p (biased) adapter dir.")
    ...
    from guardrail_ft.identify.contrasts import (
        demographic_contrast, guardrail_contrast, bias_contrast,
    )
    ...
    bias, _, _ = bias_contrast(B, Gp, task, ds, cache_base=cB, cache_guard=cG)     # NEW
    ...
    cos_bias = np.array([cosine(bias.unit_direction[i], demo_Gp.unit_direction[i]) for i in range(L)])
    mag_guard = np.array(guard.strength_per_layer)
    mag_bias = np.array(bias.strength_per_layer)
    ...
    summary = {
        ...
        "mean_abs_cos_bias_NEW": float(np.mean(np.abs(cos_bias))),
        "median_mag_ratio_guard_over_demo": float(np.median(mag_guard) / max(np.median(mag_demo), 1e-9)),
        "median_mag_ratio_bias_over_demo": float(np.median(mag_bias) / max(np.median(mag_demo), 1e-9)),
    }
    ...
    if args.out_json:
        Path(args.out_json).write_text(json.dumps({
            **summary,
            ...
            "cos_bias_NEW": cos_bias.tolist(),
        }, indent=2))

    # ----- vector geometry figure (how the vectors move) ------------------ #
    _plot_vectors(layers, cos_guard, cos_bias, args.vectors_out)

    # ----- figure (new file; existing figures untouched) ------------------ #
    fig, (axa, axb) = plt.subplots(1, 2, figsize=(12, 4.6))
    fig.suptitle("Old vs new bias estimator: mean-of-differences recovers the demographic signal",
    ...
    axa.plot(layers, cos_bias, "-o", ms=3, color="#197",
             label=f"NEW: mean-of-differences  (mean|cos|={summary['mean_abs_cos_bias_NEW']:.2f})")
    axa.set_ylim(-1.05, 1.05)
    axa.set_title("(a) Alignment with the demographic axis, per layer")
    axa.set_xlabel("layer"); axa.set_ylabel("cosine with v_demo"); axa.legend(fontsize=8, loc="lower right")

    axb.plot(layers, mag_guard, "-o", ms=3, color="#c40", label="OLD ‖v_guard‖ (grand-mean diff)")
    axb.plot(layers, mag_bias, "-o", ms=3, color="#197", label="NEW ‖v_bias‖ (mean-of-diffs)")
    axb.plot(layers, mag_demo, "--", color="#06c", label="‖v_demo‖ (reference)")
    axb.set_yscale("log")
...
def _plot_vectors(layers, cos_guard, cos_bias, out):
    """Draw v_demo, v_guard (old) and v_bias (new) as unit vectors at the true
    angular separation from the demographic axis, for an early / mid / deep layer.
    Shows the new estimator's vector rotating toward v_demo with depth."""
    import math

    layers = np.asarray(layers)
    L = len(layers)
    deep = int(np.argmax(np.abs(cos_bias)))                     # best-aligned (deep) layer
    pick = sorted({L // 4, L // 2, deep})
    fig, axes = plt.subplots(1, len(pick), figsize=(4.6 * len(pick), 4.8))
    if len(pick) == 1:
        axes = [axes]
    fig.suptitle("How the estimator moves the vector: v_bias rotates toward the demographic axis",
                 fontsize=13, fontweight="bold")

    for ax, i in zip(axes, pick):
        ang_g = math.degrees(math.acos(max(-1.0, min(1.0, cos_guard[i]))))
        ang_p = math.degrees(math.acos(max(-1.0, min(1.0, cos_bias[i]))))
        ax.set_aspect("equal"); ax.set_xlim(-1.15, 1.15); ax.set_ylim(-0.15, 1.2); ax.axis("off")
        # v_demo reference along +x
        ax.annotate("", xy=(1, 0), xytext=(0, 0), arrowprops=dict(arrowstyle="-|>", color="#06c", lw=3))
        ax.text(1.02, 0, "v_demo", color="#06c", va="center", fontsize=10)
        # v_guard (old)
        ...
        ax.text(0.02, 1.08, f"v_guard (old): {ang_g:.0f}°", color="#c40", fontsize=9)
        ax.text(0.02, 1.0, f"v_bias (new): {ang_p:.0f}°", color="#197", fontsize=9)
        kind = "deep / decision layer" if i == deep else ("early layer" if i == pick[0] else "mid layer")
        ax.set_title(f"layer {layers[i]}  ({kind})")

    fig.text(0.5, 0.0, "Unit vectors at their true angle to the demographic axis. Old v_guard stays "
             "≈90° (orthogonal); new v_bias rotates toward v_demo, most in the deep layers.",
