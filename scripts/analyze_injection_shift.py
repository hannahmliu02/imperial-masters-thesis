...
each method's erosion outcome (retained cosine AND presence).
...
Two ways to point it at a model:
  (a) --run-dir <erosion run dir>  — reads config.resolved.yaml + guardrails/G_p from
      the dir, writes injection_shift.json into it. Generic across model sizes; use
      this to loop over the Qwen ladder (see scripts/pbs/injection_shift_qwen.pbs).
  (b) --configs ... --gp-checkpoint ...  — explicit config list (original Mistral form).

    python scripts/analyze_injection_shift.py --run-dir runs/erosion_ladder_3b_s0_3860820 \
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-dir", default=None,
                    help="erosion run dir: use its config.resolved.yaml + guardrails/G_p")
    ap.add_argument("--configs", nargs="+", help="explicit config list (if not using --run-dir)")
    ap.add_argument("--set", dest="overrides", action="append", default=[])
    ap.add_argument("--gp-checkpoint", help="path to a run's guardrails/G_p (if not using --run-dir)")
    ...
    from guardrail_ft.utils.config import load_config, load_yaml, get
    ...
    if args.run_dir:
        ...
        gp_checkpoint = str(rd / "guardrails" / "G_p")
        if not Path(gp_checkpoint).exists():
            raise SystemExit(f"no G_p adapter at {gp_checkpoint}")
        out = Path(args.out) if args.out else rd
    else:
        if not (args.configs and args.gp_checkpoint):
            ap.error("pass --run-dir, or both --configs and --gp-checkpoint")
        cfg = load_config(args.configs, args.overrides)
        gp_checkpoint = args.gp_checkpoint
        out = Path(args.out) if args.out else Path("runs/injection_shift")
    ...
    Gp = make_base_loader(cfg, init_checkpoint=gp_checkpoint)()
    ...
    result = {
        "gp_checkpoint": gp_checkpoint, "data": args.data,
