#!/usr/bin/env python3
"""Visualise the d_demo vs d_guard misalignment from a study's candidate.json.

Three panels:
  (a) per-layer cosine(d_demo, d_guard) -> scatters around 0 (no alignment);
  (b) per-layer magnitudes -> ||d_guard|| >> ||d_demo|| (generic FT shift dominates);
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Plot d_demo vs d_guard misalignment.")
    ...
    fig.suptitle(args.title or "Poisoned-guardrail identification: d_demo vs d_guard are misaligned",
    ...
    axa.set_xlabel("layer"); axa.set_ylabel("cosine(d_demo, d_guard)")
    axa.set_ylim(-1, 1); axa.legend(loc="upper right", fontsize=8)

    # (b) per-layer magnitudes (log)
    axb.plot(layers, guard, "-o", ms=3, color="#c40", label="‖d_guard‖ (B → G_p shift)")
    axb.plot(layers, demo, "-o", ms=3, color="#06c", label="‖d_demo‖ (demographic axis)")
    axb.set_yscale("log")
    axb.set_title(f"(b) Magnitudes\n‖d_guard‖ ≈ {mag_ratio:.0f}× ‖d_demo‖ (generic shift dominates)")
    ...
    axc.text(1.02, 0.0, "d_guard", color="#c40", va="center", fontsize=10)
    axc.text(math.cos(th) - 0.05, math.sin(th) + 0.05, "d_demo", color="#06c", fontsize=10)
    ...
    fig.text(0.5, -0.02,
             ...
             "the demographic subspace to isolate the poisoned component.",
